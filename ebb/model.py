...
class DictActor(nn.Module):
  ...
  def forward(self,
    ...
    for key, space in self.action_space.spaces.items():
      ...
      logits = logits.view(b2 // self.n_players, self.n_players, n_actions,
                           action_dim)
...
class BaselineLayer(nn.Module):
  ...
  def __init__(self, in_channels: int, reward_space: RewardSpec, n_players=2):
...
class BasicActorCriticNetwork(nn.Module):
  ...
  def __init__(self,
    ...
    self.actor = DictActor(hidden_dim,
  ...
  def forward(self,
    ...
    if self.critic_base_model is None:
...
def create_model(flags,
  ...
  if flags.reward_schema == 'game_win_loss3':
    reward_spec = RewardSpec(
        reward_min=-1.5,
        reward_max=+1.5,
        zero_sum=True,
...
def _create_model(observation_space,
  ...
  model = BasicActorCriticNetwork(actor_base_model,
                                  critic_base_model,
                                  hidden_dim,
                                  base_out_channels,
                                  reward_spec,
