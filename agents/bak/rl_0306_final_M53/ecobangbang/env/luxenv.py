...
class LuxS3Env(gym.Env):
  ...
  def _convert_observation(self, ob, mm, final_state=None, skip_check=False):
    ...
    if self.obs_space_kwargs.get('hide_relic'):
      o['solved_relic'][hidden_relic_mask] = 0.1
      # o['_a_team_point_prob'][hidden_relic_mask] = 0.0
      o['_a_team_point_prob'][hidden_relic_mask] *= 0.1
  ...
  def _convert_win_loss_reward4(self, raw_obs, env_state):
    wt = self.reward_shaping_params
  ...
  def _convert_reward(self, raw_obs, env_state):
    ...
    if self.reward_schema == 'game_win_loss4':
      reward = self._convert_win_loss_reward4(raw_obs, env_state)
  ...
  def get_info(self,
    ...
    def _get_info(agent_action, raw_obs1, prev_obs1, agent_reward, mm,
      ...
      info['_match_played_0'] = 0
      info['_match_played_1'] = 0
