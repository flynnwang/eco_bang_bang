...
MODEL_FILE_NAME = "069065216_weights.pt"
...
class Agent:
  ...
  def convert_observation(self, raw_obs):
    ...
    action_mask = self._available_action_mask
  ...
  def act(self, step: int, raw_obs, remainingOverageTime: int = 60):
    ...
    print(f'step={step}, player={self.player}', file=sys.stderr)
    ...
    model_output = self.md(model_input, sample=DO_SAMPLE, probs_output=True)
    action_probs = model_output["actions"][UNITS_ACTION][0]
